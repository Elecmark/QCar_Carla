...
def apply_model_delta_to_vehicle(
    ...
    """
    v4 的策略（核心与前几版不同）：
    - 完全在“模型坐标系”里演化 state_model = state_model + Δstate_model。
    - 对 CARLA 只应用【平面位移 Δx, Δy 以及 yaw 的增量】，z/roll/pitch 交给物理和当前道路。
      这样既尊重了训练好的 LSTM，又不会因为 z 或姿态偏差导致穿地或飞起。
    """
    tf = vehicle.get_transform()

    # 1) 在模型空间中的位移和朝向变化
    prev_xy = curr_model_state[0:2]
    next_xy = next_model_state[0:2]
    delta_xy = next_xy - prev_xy  # [Δx, Δy]（模型空间）

    # yaw 变化：从两个四元数中求 yaw，再取差值
    qw0, qx0, qy0, qz0 = curr_model_state[3:7]
    qw1, qx1, qy1, qz1 = next_model_state[3:7]
    _, _, yaw_deg_0 = quat_wxyz_to_euler_deg(float(qw0), float(qx0), float(qy0), float(qz0))
    _, _, yaw_deg_1 = quat_wxyz_to_euler_deg(float(qw1), float(qx1), float(qy1), float(qz1))
    delta_yaw = yaw_deg_1 - yaw_deg_0

    # 2) 把模型的 Δx,Δy 直接叠加到当前 CARLA 位置（假设坐标系一致）
    new_x = tf.location.x + float(delta_xy[0])
    new_y = tf.location.y + float(delta_xy[1])
    new_z = tf.location.z  # 不改 z，完全交给当前道路 & 物理

    new_yaw = tf.rotation.yaw + float(delta_yaw)
    new_tf = carla.Transform(
        carla.Location(x=new_x, y=new_y, z=new_z),
        carla.Rotation(roll=tf.rotation.roll, pitch=tf.rotation.pitch, yaw=new_yaw),
    )
    vehicle.set_transform(new_tf)

    # 3) 用 Δx,Δy,Δz 近似线速度，驱动物理（可选但有助于速度一致）
    dx = float(delta_xy[0])
    dy = float(delta_xy[1])
    dz = 0.0  # 我们没有用模型的 Δz 来改高度
...
# ============================================================
# 7) 简单的轨迹记录与可视化
#    - 在 v4 运行过程中记录 CARLA 的 pos_x,pos_y,pos_z
#    - 运行结束后，把 CARLA 轨迹和 forward_evaluation_v1 中的真实轨迹画成两张图
# ============================================================
def save_and_plot_trajectories(
    ...
    out_csv = os.path.join(project_root, "carla_run_traj_v4.csv")

    # 写出 CARLA 运行轨迹，方便后续单独分析
    try:
        ...
        print(f"[traj] 已写出 CARLA 运行轨迹: {out_csv}")
    except Exception as e:
        print(f"[traj] 写出 CARLA 轨迹 CSV 失败: {e}")

    # 如果没有 pandas / matplotlib，就只写 CSV，不画图
    if pd is None or plt is None:
        print("[traj] 未安装 pandas 或 matplotlib，跳过轨迹可视化。")
        return

    try:
        carla_df = pd.DataFrame(log_rows)

        eval_dir = os.path.join(
            project_root, "results_evaluation_v1", "forward_evaluation"
        )
        pos_x_path = os.path.join(eval_dir, "forward_pos_x_error.csv")
        pos_y_path = os.path.join(eval_dir, "forward_pos_y_error.csv")
        pos_z_path = os.path.join(eval_dir, "forward_pos_z_error.csv")

        pos_x_df = pd.read_csv(pos_x_path)
        pos_y_df = pd.read_csv(pos_y_path)
        pos_z_df = pd.read_csv(pos_z_path)

        # -------- 图 1：CARLA 中汽车的 pos_x,pos_y,pos_z 曲线 --------
        plt.figure(figsize=(10, 6))

        plt.subplot(3, 1, 1)
        plt.plot(carla_df["time"], carla_df["carla_pos_x"], label="CARLA pos_x")
        plt.ylabel("pos_x (m)")
        plt.legend()
        plt.grid(True)

        plt.subplot(3, 1, 2)
        plt.plot(carla_df["time"], carla_df["carla_pos_y"], label="CARLA pos_y")
        plt.ylabel("pos_y (m)")
        plt.legend()
        plt.grid(True)

        plt.subplot(3, 1, 3)
        plt.plot(carla_df["time"], carla_df["carla_pos_z"], label="CARLA pos_z")
        plt.xlabel("time (s)")
        plt.ylabel("pos_z (m)")
        plt.legend()
        plt.grid(True)
        ...
        plt.tight_layout()
        plt.suptitle("CARLA 车辆轨迹 (pos_x,pos_y,pos_z)", y=1.02)

        # -------- 图 2：模型数据中的 pos_x,pos_y,pos_z 曲线（真实 vs 预测） --------
        t_eval = pos_x_df["Time Step"]

        plt.figure(figsize=(10, 8))

        plt.subplot(3, 1, 1)
        plt.plot(t_eval, pos_x_df["True pos_x"], label="True pos_x")
        plt.plot(t_eval, pos_x_df["Predicted pos_x"], label="Pred pos_x", alpha=0.7)
        plt.ylabel("pos_x (m)")
        plt.legend()
        plt.grid(True)

        plt.subplot(3, 1, 2)
        plt.plot(t_eval, pos_y_df["True pos_y"], label="True pos_y")
        plt.plot(t_eval, pos_y_df["Predicted pos_y"], label="Pred pos_y", alpha=0.7)
        plt.ylabel("pos_y (m)")
        plt.legend()
        plt.grid(True)

        plt.subplot(3, 1, 3)
        plt.plot(t_eval, pos_z_df["True pos_z"], label="True pos_z")
        plt.plot(t_eval, pos_z_df["Predicted pos_z"], label="Pred pos_z", alpha=0.7)
        ...
        plt.tight_layout()
        plt.suptitle("模型数据轨迹 (True vs Pred, pos_x,pos_y,pos_z)", y=1.02)

        plt.show()
    except Exception as e:
        print(f"[traj] 绘制轨迹图失败: {e}")
...
# ============================================================
# 10) “模型历史窗口”初始化
#      这里非常关键：我们维护一个【纯模型空间】的 state_model_history，
#      它与 CARLA 的真实 transform 解耦，只在初始时对齐，一旦开始自动模式，
#      以后完全按照 LSTM 的输出演化。
# ============================================================
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="CARLA + QCar World Model Auto Driver (v4, pure-model-space Δstate)"
    )
    ...
    pygame.init()
    pygame.display.set_caption("QCar World Model Auto Driver v4")
    pygame.display.set_mode((860, 230))
    ...
    try:
        ...
        print("=" * 80)
        print("脚本 v4 已启动（模型空间 Δstate 推理）")
        print("模型输入特征：")
        ...
        while True:
            ...
            if key_reader.just_pressed(VK_W):
                auto_mode = True
                latched_action = np.array([0.06, 0.00], dtype=np.float32)
                model_state = extract_state_vector_from_vehicle(vehicle)
                bootstrap_model_history(
            ...
            if auto_mode and len(history) == normalizer.seq_length:
                ...
                apply_model_delta_to_vehicle(
                    vehicle=vehicle,
                    curr_model_state=model_state,
        ...
        # 在资源清理之后进行轨迹保存与可视化
        try:
            project_root_safe = find_project_root()
            save_and_plot_trajectories(log_rows, project_root_safe)
        except Exception as e:
            print(f"[traj] 轨迹保存与绘制过程出错: {e}")
